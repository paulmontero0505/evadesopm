...
function radio_records_sql(): string {
    return "SELECT ra.*, COALESCE(ra.delivery_group, CONCAT('legacy-', ra.id)) AS group_id, r.code AS radio_code, r.imei, r.model, u.full_name AS supervisor_name, u.role AS supervisor_role, custodian.full_name AS current_supervisor_name, creator.full_name AS registered_by_name, returner.full_name AS returned_by_name, o.id AS collaborator_id, o.code AS collaborator_code, o.full_name AS collaborator_name FROM radio_assignments ra JOIN radios r ON r.id=ra.radio_id JOIN users u ON u.id=ra.supervisor_id LEFT JOIN users custodian ON custodian.id=COALESCE(ra.current_supervisor_id, ra.supervisor_id) JOIN users creator ON creator.id=ra.registered_by LEFT JOIN users returner ON returner.id=ra.returned_by LEFT JOIN radio_assignment_collaborators rac ON rac.radio_assignment_id=ra.id LEFT JOIN opms o ON o.id=rac.opm_id ";
}
