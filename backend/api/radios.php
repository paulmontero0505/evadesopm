...
function handle_radio_context(): void {
    ...
    $next=db()->prepare("SELECT u.id AS user_id, u.full_name, u.role, CASE WHEN current_assignment.id IS NULL THEN 0 ELSE 1 END AS in_turn, CASE WHEN next_assignment.id IS NULL THEN 0 ELSE 1 END AS in_next_turn FROM users u LEFT JOIN supervisor_assignments current_assignment ON current_assignment.user_id=u.id AND current_assignment.work_date=? AND current_assignment.turno=? LEFT JOIN supervisor_assignments next_assignment ON next_assignment.user_id=u.id AND next_assignment.work_date=? AND next_assignment.turno=? WHERE u.active=1 AND u.role IN ('supervisor','coordinator') ORDER BY CASE WHEN current_assignment.id IS NULL THEN 1 ELSE 0 END, u.full_name"); $next->execute([$date,$turno,$nextDate,$nextTurno]);
