...
const EMPTY = { person_key: '', funcion_1: '', funcion_2: '', zona_1: '', zona_2: '', puesto: '', nave: '', nave_2: '' }
...
export default function Asignaciones() {
  ...
  const [confirmingTurno, setConfirmingTurno] = useState(false); const [pendingFile, setPendingFile] = useState(null); const [toast, setToast] = useState('')
  ...
  async function selectFile(event) {
    ...
    if (!file) return
    setPendingFile(file)
    setConfirmingTurno(true)
  }
  async function runImport() {
    if (!pendingFile) return
    setConfirmingTurno(false)
    setImporting(true); setError(''); setMessage(''); setToast('')
    try {
      const result = await api.importAssignments(pendingFile, turno, date)
      const omitted = result.errors?.length ? ` Se omitieron ${result.errors.length} filas.` : ''
      setMessage(`Se importaron ${result.imported} asignaciones.${omitted}`)
      setToast('Completado!')
      await load()
    } catch (err) { setError(err.message) } finally { setImporting(false); setPendingFile(null) }
  }
  ...
  return <><TopBar title={t.asignacionesTitulo} to="/turno" /><main className="content assignment-page">
    ...
    <section className="card assignment-setup"><h3>{t.cargarAsignaciones}</h3><p className="muted assignment-copy">Importe una sola plantilla con colaboradores, supervisores y coordinadores. Se leen solo apellidos y nombres, función, zona, puesto y naves; las demás columnas del libro operativo se ignoran y no aparecen en el resultado.</p>
    ...
    <section className="assignment-list assignment-results"><div className="assignment-list-heading"><div><h2>{cargo ? `Asignaciones: ${cargo}` : 'Personal asignado'}</h2><p>{date} · {turnoText(turno, lang)}</p></div><span className="chip"><UsersRound size={14} /> {filteredAssignments.length}</span></div><label className="assignment-search"><Search size={16} /><input className="input" placeholder="Buscar por nombre o cargo" value={search} onChange={(event) => setSearch(event.target.value)} /></label>
      {loading ? <div className="empty">{t.cargando}</div> : filteredAssignments.length === 0 ? <div className="assignment-empty"><CalendarDays size={25} /><div><strong>No hay asignaciones</strong><span>Importe una plantilla o registre una asignación individual.</span></div></div> : <div className="assignment-results-list">{filteredAssignments.map((row) => <article className="assignment-row" key={`${row.person_type}:${row.id}`}><div className="assignment-row-main"><strong>{row.full_name}</strong><span>{row.code ? `${row.code} · ` : ''}{peopleByKey.get(personKey(row.person_type, row.person_id))?.puesto || row.puesto || roleLabel(row)}</span></div><div className="assignment-row-detail"><strong>{row.funcion_1 || t.sinFuncion}</strong><span>{[row.zona_1, row.zona_2, row.nave].filter(Boolean).join(' · ') || 'Sin zona o nave'}</span></div></article>)}</div>}
    </section>
    {confirmingTurno && <div className="turno-confirm-backdrop" role="dialog" aria-modal="true" aria-labelledby="turno-confirm-title"><div className="turno-confirm"><h3 id="turno-confirm-title">Confirmar turno</h3><p className="muted">¿Es correcto el turno seleccionado para importar las asignaciones?</p><div className="turno-confirm-info"><span>Fecha</span><strong>{date}</strong><span>Turno</span><strong>{turnoText(turno, lang)}</strong></div><div className="turno-confirm-actions"><button className="btn secondary" onClick={() => { setConfirmingTurno(false); setPendingFile(null) }}>Cancelar</button><button className="btn" onClick={runImport}>Sí, importar</button></div></div></div>}
  </main></>
}
...
export function SearchablePicker({ items, value, onSelect, labelOf, searchOf, placeholder, statusOf, statusClassOf, extraItems = [] }) { const [query, setQuery] = useState(''); const [open, setOpen] = useState(false); const allItems = [...items, ...extraItems]; const selected = allItems.find((item) => String(item.person_key || item.id) === String(value)); const text = open ? query : (selected ? labelOf(selected) : query); const filteredMatches = query.trim() ? items.filter((item) => searchOf(item).toLowerCase().includes(query.trim().toLowerCase())).slice(0, 30) : []; const matches = [...filteredMatches, ...extraItems]; function select(item) { onSelect(item.person_key || item.id); setQuery(labelOf(item)); setOpen(false) } return <div className="search-picker"><div className="search-picker-input"><Search size={16} /><input className="input" value={text} onFocus={() => { setQuery(selected ? labelOf(selected) : query); setOpen(true) }} onChange={(event) => { setQuery(event.target.value); onSelect(''); setOpen(true) }} onBlur={() => setTimeout(() => setOpen(false), 120)} placeholder={placeholder} autoComplete="off" /></div>{open && <div className="search-picker-menu">{!query.trim() && <div className="search-picker-empty">Escriba para buscar.</div>}{matches.map((item) => <button type="button" className={`search-picker-option${item.isExtra ? ' search-picker-option-unknown' : ''}`} key={item.person_key || item.id} onMouseDown={(event) => { event.preventDefault(); select(item) }}><span>{labelOf(item)}</span>{statusOf?.(item) && <span className={`search-picker-option-status${statusClassOf?.(item) ? ` ${statusClassOf(item)}` : ''}`}>{statusOf(item)}</span>}</button>)}{query.trim() && !matches.length && <div className="search-picker-empty">No se encontraron coincidencias.</div>}</div>}</div> }
export function AssignmentFields({ form, update, hidePuesto = false }) { return <div className="radio-form-grid"><div><label>Función 1</label><input className="input" value={form.funcion_1} onChange={(event) => update('funcion_1', event.target.value)} /></div><div><label>Función 2</label><input className="input" value={form.funcion_2} onChange={(event) => update('funcion_2', event.target.value)} /></div><div><label>Zona 1</label><input className="input" value={form.zona_1} onChange={(event) => update('zona_1', event.target.value)} /></div><div><label>Zona 2</label><input className="input" value={form.zona_2} onChange={(event) => update('zona_2', event.target.value)} /></div>{!hidePuesto && <div><label>Puesto</label><input className="input" value={form.puesto} onChange={(event) => update('puesto', event.target.value)} /></div>}<div><label>Nave</label><input className="input" value={form.nave} onChange={(event) => update('nave', event.target.value)} /></div><div><label>Nave 2</label><input className="input" value={form.nave_2} onChange={(event) => update('nave_2', event.target.value)} /></div></div> }
