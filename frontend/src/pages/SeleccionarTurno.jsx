  ...
  return <><TopBar title={t.turnoTitulo} onExit={salir} /><main className="content shift-page">
    ...
    <section className="shift-opm-list" aria-labelledby="shift-roster-title">
      <div className="assignment-list-heading shift-selection-heading"><div><h2 id="shift-roster-title">Equipo del turno</h2><p>{date} · {turnoText(turno, lang)} · Seleccione todo el personal o filtre por cargo.</p></div><span className="shift-selection-count"><UsersRound size={15} /> <strong>{selected.size}</strong> seleccionados</span></div>
       <div className="assignment-selection-tools"><select className="input" aria-label="Filtrar por cargo" value={puesto} onChange={(event) => setPuesto(event.target.value)}><option value="">Todos los cargos</option>{puestos.map((item) => <option key={item} value={item}>{item}</option>)}</select><select className="input" aria-label="Filtrar por ubicación" value={ubicacion} onChange={(event) => changeUbicacion(event.target.value)}><option value="">Todas las ubicaciones</option>{ubicaciones.map((item) => <option key={item} value={item}>{item}</option>)}</select><label className="shift-search"><Search size={17} /><input type="text" className="input" placeholder="Buscar por nombre, código o cargo" value={filterText} onChange={(event) => setFilterText(event.target.value)} /></label><div className="assignment-filter-checks"><label className="assignment-select-all"><input type="checkbox" checked={allSelected} onChange={(event) => toggleAll(event.target.checked)} disabled={!selectable.length} /><span>Seleccionar todos ({selectable.length})</span></label><label className="assignment-select-all"><input type="checkbox" checked={onlyInTurn} onChange={(event) => setOnlyInTurn(event.target.checked)} /><span>Solo en turno</span></label><label className="assignment-select-all"><input type="checkbox" checked={onlyBirthday} onChange={(event) => setOnlyBirthday(event.target.checked)} /><span>Cumpleaños</span></label></div></div>
