...
function RadioMultiPicker({ radios, selectedIds, onChange }) {
  ...
  const matches = radios.filter((radio) => Number(radio.available) && !selectedIds.includes(radio.id) && (!query.trim() || `${radio.code} ${radio.imei} ${radio.model}`.toLowerCase().includes(query.trim().toLowerCase()))).sort((a, b) => Number(a.code) - Number(b.code) || String(a.code).localeCompare(String(b.code), undefined, { numeric: true })).slice(0, 20)
