...
export default function ControlRadios() {
  ...
  const { user } = useAuth()
  const [data, setData] = useState({ radios: [], opms: [], supervisors: [], records: [], locations: [] })
  ...
  const allPuestos = useMemo(() => [...availablePuestos, ...Object.keys(puestoCounts).filter((puesto) => !puestoAvailability[puesto])], [availablePuestos, puestoAvailability, puestoCounts])
  ...
  return <><TopBar title="Trazabilidad de equipos / radios" to="/" onBack={module ? () => { setModule(null); setError(''); setMessage('') } : undefined} /><main className="content">
    ...
    {canCreate && module === 'deliver' && showDeliveryForm && <section className="card"><div className="assignment-list-heading"><div><h3>{editingGroup ? `Editar entrega: ${radioIds.length} radios` : 'Registrar entrega de radios'}</h3><p className="muted">{shift.date} · {turnoLabel(shift.turno)}</p></div><span className="chip"><Radio size={14} /> {radioIds.length}</span></div>
      {loading ? <div className="empty">Cargando radios y cuadrilla del turno…</div> : <form onSubmit={submit}>
        ...
        <Picker title="Radios a entregar" description={editingGroup ? 'Esta es una sola entrega: las radios quedan agrupadas y puede actualizar sus estados.' : 'Escriba para buscar radios y agréguelos a la entrega.'} count={radioIds.length}>
        ...
        <Picker title="Puestos que recibirán radios" description="Seleccione los puestos detectados o agregue uno manualmente. Use − / + para indicar cuántas radios recibirá." count={totalPuestos}>
    ...
    {module === 'relief' && <GroupedReliefPanel records={data.relief_records || []} allRecords={data.records} opms={data.opms} user={user} nextSupervisors={data.next_supervisors || []} nextShift={data.next_shift} onReload={load} />}
